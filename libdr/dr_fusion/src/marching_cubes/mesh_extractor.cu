...
namespace refusion 
{
    namespace tsdfvh 
    {
                                                                                        ...
                                                                                        uchar3 &color) 
        {
            ...
            v = volume->GetVoxel(pos_dual + make_float3(voxel_size, voxel_size, voxel_size));
            if (v.weight == 0) 
                return false;
            vColor = make_float3(v.color.x, v.color.y, v.color.z);
        ...
        {
            Vertex r1; r1.position = p1; 
            r1.color = make_float3(c1.x, c1.y, c1.z) / 255.f;
            Vertex r2; r2.position = p2; 
            r2.color = make_float3(c2.x, c2.y, c2.z) / 255.f;

            if (fabs(isolevel - d1) < 0.00001f) 
                return r1;
            if (fabs(isolevel - d2) < 0.00001f) 
                return r2;
            if (fabs(d1 - d2) < 0.00001f) 
                return r1;
            ...
            // Color
            res.color.x = static_cast<float>(c1.x + mu * static_cast<float>(c2.x - c1.x)) / 255.f;
            res.color.y = static_cast<float>(c1.y + mu * static_cast<float>(c2.y - c1.y)) / 255.f;
            res.color.z = static_cast<float>(c1.z + mu * static_cast<float>(c2.z - c1.z)) / 255.f;
                                                                                    ...
                                                                                    Mesh *mesh) 
        {
            ...
            int3 grid_size = make_int3(size.x / voxel_size, size.y / voxel_size, size.z / voxel_size);
            int grid_linear_size = grid_size.x * grid_size.y * grid_size.z;
            for (int i = index; i < grid_linear_size; i += stride) 
            {
                // Delinearize index
                int3 grid_position = make_int3(i % grid_size.x, (i / grid_size.x) % grid_size.y, i / (grid_size.x * grid_size.y));
                float3 world_position = make_float3(   static_cast<float>(grid_position.x) * voxel_size + lower_corner.x,
        ...
        void MeshExtractor::ExtractMesh(TsdfVolume *volume, 
                                                                            float3 lower_corner,
                                                                            float3 upper_corner, cudaStream_t stream) 
        {
            int threads_per_block = 256;
            int thread_blocks =(volume->GetOptions().num_blocks * volume->GetOptions().block_size +threads_per_block - 1) /threads_per_block;
            if (stream == nullptr) 
            {
        ...
        __device__ void ExtractMeshWithInstanceAtPosition(  TsdfVolume *volume,
                                                                                                                        const float3 &position, float voxel_size,
                                                                                                                        Mesh *mesh,
                                                                                                                        bool is_instance,
                                                                                                                        unsigned char* instanceColorList_gpu) 
        {
            ...
            uchar3 color;
            //   uchar3 color = v.color;
            // uchar3 color = v.instance_color;
            //   uchar3 color = make_uchar3(200,200,200);
            if(is_instance)
            {
                if(v.InstanceIndex !=0 && v.InstanceFrameNum>1)
                {
                    color = make_uchar3(instanceColorList_gpu[3*(v.InstanceIndex-1)+0],
        ...
        __global__ void ExtractMeshWithInstanceKernel( TsdfVolume *volume, 
                                                                                                              float3 lower_corner,
                                                                                                              float3 upper_corner, 
                                                                                                              float voxel_size,
                                                                                                              Mesh *mesh, 
                                                                                                              ...
                                                                                                              unsigned char* instanceColorList_gpu)  
        {
            ...
            for (int i = index; i < grid_linear_size; i += stride) 
            {
                ...
                float3 world_position = make_float3(   static_cast<float>(grid_position.x) * voxel_size + lower_corner.x,
                                                                                                static_cast<float>(grid_position.y) * voxel_size + lower_corner.y,
                                                                                                static_cast<float>(grid_position.z) * voxel_size + lower_corner.z);
                ExtractMeshWithInstanceAtPosition(volume, world_position, voxel_size, mesh,is_instance,instanceColorList_gpu);
            }
        }
 


        void MeshExtractor::ExtractMeshWithInstance(TsdfVolume *volume, 
                                                                                                        float3 lower_corner,
                                                                                                        float3 upper_corner, 
                                                                                                        bool is_instance,
                                                                                                        unsigned char* instanceColorList_gpu ,
                                                                                                        cudaStream_t stream) 
        {
            ...
            if (stream == nullptr) 
            {
                ExtractMeshWithInstanceKernel<<<thread_blocks, threads_per_block>>>(
                    volume, lower_corner, upper_corner, voxel_size_, mesh_,is_instance,instanceColorList_gpu);
            }
            else
            {
                ExtractMeshWithInstanceKernel<<<thread_blocks, threads_per_block, 0, stream>>>(
                    volume, lower_corner, upper_corner, voxel_size_, mesh_,is_instance,instanceColorList_gpu);
            }
        }
