 ...
 */
        ...
        public:
            ...
            std::vector<InstanceBlock> InstanceTable;
            unsigned char* InstanceMasks_gpu;

            unsigned char* InstanceColorList_gpu  = NULL;
            ...
            __host__ __device__ bool UpdateSemanticVoxel(float3 position, const Voxel &voxel);
            /**
            ...
            Mesh ExtractMeshWithInstance(const float3 &lower_corner, const float3 &upper_corner,bool is_instance); 


            void ExtractMeshAsync(const float3 &lower_corner, const float3 &upper_corner);
            void ExtractMeshWithInstanceAsync(const float3 &lower_corner, const float3 &upper_corner,bool is_instance);
            ...
            void IntegrateScanWithSemanticAsync(
            ...
            void RenderWithMaskAsync(std::vector<float4x4> camera_poses, RgbdSensor sensor);
            /**
            ...
            void GetRenderWithMaskResult(std::vector<unsigned char *> &bgr, std::vector<unsigned char *> &instance_bgr, std::vector<float *> &depth,std::vector<cv::Mat>& InstanceMask);
        ...
        protected:
            ...
            __host__ std::vector<uchar3 *> &h_instance_bgr_render_blocked();

            __host__ std::vector<float *> &h_depth_render_blocked();

            __host__ std::vector<uchar3 *> &h_bgr_render_free();
            
            __host__ std::vector<uchar3 *> &h_instance_bgr_render_free();
            ...
            std::vector<uchar3 *> d_instance_bgr_render_;
            std::vector<float *> d_depth_render_;

            std::pair<std::vector<uchar3 *>, std::vector<uchar3 *>> h_bgr_render_;
            std::pair<std::vector<uchar3 *>, std::vector<uchar3 *>> h_instance_bgr_render_;
