...
void DrFusion::PrintInstanceTable()
{
    volume_->PrintInstanceTable();
}

void DrFusion::InstanceTableAdd(unsigned char InstanceClass) {
    volume_->InstanceTableAdd(InstanceClass);
    // PrintInstanceTable();
}

int DrFusion::GetInstanceNum()
{
    return volume_->GetInstanceNum();
}

void DrFusion::GetInstanceColor(unsigned int InstanceIndex,cv::Scalar& color)
{
    volume_->GetInstanceColor(InstanceIndex,color);
}

void DrFusion::UpdateInstanceTable(int InstanceIndex,int ClassIndex)
{
    volume_->UpdateInstanceTable(InstanceIndex,ClassIndex);
}
...
void DrFusion::IntegrateScanWithSemanticAsync(unsigned char *bgr, float *depth,unsigned char* masks, int masks_num,float const *pose) 
{
  refusion::float4x4 pose_cuda(pose);
  volume_->IntegrateScanWithSemanticAsync(*sensor_, bgr, depth, masks,masks_num,pose_cuda);
}
...
void DrFusion::RenderWithMaskAsync(std::vector<float const *> camera_poses) {
  std::vector<refusion::float4x4> poses_cuda;
  for (auto const &p: camera_poses)
    poses_cuda.push_back(refusion::float4x4(p));

  volume_->RenderWithMaskAsync(poses_cuda, *sensor_);
}
...
void DrFusion::GetRenderWithMaskResult(std::vector<unsigned char *> &bgr, std::vector<unsigned char *> &instance_bgr, std::vector<float *> &depth,std::vector<cv::Mat>& InstanceMasks) {
  volume_->GetRenderWithMaskResult(bgr, instance_bgr, depth,InstanceMasks);
}
...
//    Synchronize();
  ...
  refusion::tsdfvh::Mesh mesh = volume_->ExtractMeshWithInstance(ToFloat3(lower_corner), ToFloat3(upper_corner),true);
...
void DrFusion::ExtractMeshWithInstanceAsync(float *lower_corner, float *upper_corner,bool is_instance) {
#ifdef DR_FUSION_DEBUG
  auto start = std::chrono::high_resolution_clock::now();
#endif

  volume_->ExtractMeshWithInstanceAsync(ToFloat3(lower_corner), ToFloat3(upper_corner),is_instance);
