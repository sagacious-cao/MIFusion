...
int main(int argc, char **argv) 
{
    ...
    std::thread runthread([&]() 
    {
        ...
        for (int ii = 0; ii < (int) idsToPlay.size(); ii++) 
        {
            ...
            if (!skipFrame) 
                fullSystem->addActiveFrame(img, img_bgr, depth, dvo_img, i);

            if (!disableAllDisplay) 
            {
                if(fullSystem->outputWrapper[0]->should_save_mesh())
                {
        ...
        if (dr_timing) 
            fullSystem->printDrStatistics(opt.result_folder);

        int numFramesProcessed = abs(idsToPlay[0] - idsToPlay.back());

        system("nvidia-smi > gpu.txt");
        
        std::ofstream GpuFile("gpu.txt",std::ios_base::app); 
        GpuFile << (MilliSecondsTakenHRC / 1000.0)<<"s"<<std::endl; //把字符串内容"This is a Test!"，写入Test.txt文件
	    GpuFile << (MilliSecondsTakenHRC / numFramesProcessed)<<"ms"<<std::endl; //把字符串内容"This is a Test!"，写入Test.txt文件
        GpuFile << (1000 * numFramesProcessed / MilliSecondsTakenHRC)<<"fps"<<std::endl; //把字符串内容"This is a Test!"，写入Test.txt文件
        GpuFile.close();
