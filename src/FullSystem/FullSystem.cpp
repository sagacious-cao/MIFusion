...
FullSystem::FullSystem() {
  ...
  coarseTracker = new CoarseTracker(wG[0], hG[0], setting_tracking_device == "cuda");
  coarseTracker_forNewKF = new CoarseTracker(wG[0], hG[0], setting_tracking_device == "cuda");
  coarseInitializer = new CoarseInitializer(wG[0], hG[0]);
  coarseRgbdInitializer = new CoarseRGBDInitializer(wG[0], hG[0]);
...
void FullSystem::initDr() {
  if (mvsnet_flag) {
    int num_render_streams = (setting_tracking_type == "dense") ? 1 : 0;

    DrFusionOptions dr_options{
        .voxel_size = 0.01,
        .num_buckets = 100000,   //1000000
        .bucket_size = 10,
        .num_blocks = 100000,  //600000
        .block_size = 8,
        .max_sdf_weight = 64,
    ...
    printf("-----------------------maskrcnn initalizing---------------\n");
	maskrcnn = new SampleMaskRCNN(maskrcnnParams);
	// maskrcnn->build("weights/eigen1024fp16.bin",true);
    maskrcnn->build("weights/eigen1024fp16.bin",true);
    // maskrcnn->build("weights/eigen1024int8.bin",true);
    printf("-----------------------maskrcnn initalizing  done---------------\n");

    slicr = new gSLICrTools(dr_width,dr_height);
    slicr->initCameraIntrinsics(Hcalib.cxl(),Hcalib.cyl(),Hcalib.fxl(),Hcalib.fyl());    
    std::cout<<"cx:"<<Hcalib.cxl()<<" cy:"<<Hcalib.cyl()<<" fx:"<<Hcalib.fxl()<<" fy:"<<Hcalib.fyl()<<std::endl;

    mvsnet = new DrMvsnet((mvsnet_folder + "model.pt").c_str());
    if (!test_dr_mvsnet(*mvsnet, (mvsnet_folder + "sample_inputs.pt").c_str(), true, 4)) {
      printf("Couldn't load MVSNet successfully.");
//                exit(EXIT_FAILURE);
    }

    tandem_backend = new TandemBackend(
        dr_width, 
        dr_height, 
        ...
        outputWrapper, 
        mesh_extraction_freq,
        maskrcnn,
        slicr);
  } else {
    setting_tsdf_fusion = false;
  }
}
...
std::string zeroPadNumber(int num, int pad = 6) {
  std::stringstream ss;
...
void FullSystem::printAllResultSE3(std::string file) {
  boost::unique_lock<boost::mutex> lock(trackMutex);
  boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
  ...
  int ind = 0;
  for (FrameShell *s : allFrameHistory) {
    Eigen::Matrix<double, 3, 4> pose = s->camToWorld.matrix3x4();
    myfile << zeroPadNumber(s->id) << " "
...
void FullSystem::printKeyframeIndex(std::string file) {
  boost::unique_lock<boost::mutex> lock(trackMutex);
  boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
  ...
  for (FrameShell *s : allFrameHistory) {
    ...
    // Note that here ony the index of the marginalized KFs is saved
    // to ensure high pose precision.
    if (setting_onlyLogKFPoses && s->marginalizedAt == s->id) continue;

    if (first) {
      myfile << zeroPadNumber(s->id);
      first = false;
    } else myfile << " " << zeroPadNumber(s->id);
  }
  myfile << "\n";
  myfile.close();
}

void FullSystem::printDrStatistics(std::string folder) {
  dr_timer.write_to_file(folder + "dr_times.txt");
}

void FullSystem::printOptimizationWindowIndex(std::string file) {
  std::ofstream myfile;
  myfile.open(file.c_str());
  ...
  for (auto const &optimization_window: optimization_windows) {
    sep = "";
    for (auto id: optimization_window) {
      myfile << sep << zeroPadNumber(id);
      sep = separator;
    }
    myfile << "\n";
  }
...
Vec4 FullSystem::trackNewCoarse(FrameHessian *fh) {
  ...
  for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
    ...
    if (rgbd_flag) {
      trackingIsGood = trackingIsGood && coarseTracker->trackNewestCoarseDense(
          fh, lastF_2_fh_this, aff_g2l_this,
          pyrLevelsUsed - 1,
          achievedRes);
    }
...
void FullSystem::addActiveFrame(ImageAndExposure *image, unsigned char *image_bgr, RGBDepth *rgbd_detph,
                                dvo::core::RgbdImagePyramid *dvo_img, int id) 
{
    int time_id;
  ...
  FrameHessian *fh = nullptr;
  if (!rgbd_flag) {
    fh = new FrameHessian();
  } else {
    fh = new FrameHessian(dvo_img->camera_, dvo_img->level(0).intensity, dvo_img->level(0).depth);
  }
  ...
  if (!initialized) {
    ...
    if (!rgbd_flag) {
      ...
      // use initializer!
      if (coarseRgbdInitializer->frameID < 0)    // first frame set. fh is kept by coarseInitializer.
      {
        coarseRgbdInitializer->setFirst(&Hcalib, fh, rgbd_detph);
      } else if (coarseRgbdInitializer->trackFrame(fh, &Hcalib, rgbd_detph, outputWrapper))    // if SNAPPED
      {
        initializeFromRGBDInitializer(fh, rgbd_detph);
        lock.unlock();
        deliverTrackedFrame(fh, true, rgbd_detph->depth);
      } else {
        // if still initializing
        fh->shell->poseValid = false;
        delete fh;
  ...
  } else    // do front-end operation.
  {
    ...
    if (dr_timing)
      time_id = dr_timer.start_timing("dso-trackNewCoarse");
    Vec4 tres = trackNewCoarse(fh);
    if (dr_timing)
      dr_timer.end_timing("dso-trackNewCoarse", time_id);
    ...
    if (fh->shell->id == frame_save_index) {
      printf("\n\n\nForcing Frame Save!\n\n\n");
      needToMakeKF = true;
    }
    ...
    lock.unlock();
    if (dr_timing)
      time_id = dr_timer.start_timing("deliverTrackedFrame");
    deliverTrackedFrame(fh, needToMakeKF, rgbd_detph == nullptr ? nullptr : rgbd_detph->depth); // TODO: Here all poses are optimized
    if (dr_timing)
      dr_timer.end_timing("deliverTrackedFrame", time_id);

    // TODO: Check needKF -> pass to MVSNet
    if (mvsnet_flag) {
      if (dr_timing)
        time_id = dr_timer.start_timing("deliverDrFrame");
      deliverDrFrame(fh, id, image_bgr, image->h, image->w, needToMakeKF);
      if (dr_timing) {
        dr_timer.end_timing("deliverDrFrame", time_id);
      }
    }
...
void FullSystem::deliverTrackedFrame(FrameHessian *fh, bool needKF, float *rgbd_depth) {
  ...
  if (linearizeOperation) {
    ...
    if (needKF) makeKeyFrame(fh, rgbd_depth);
...
void FullSystem::deliverDrFrame(FrameHessian *fh, int id, unsigned char *&image_bgr, int height, int width, bool needKF) {
  ...
  if (dr_mvsnet_view_num == 7 && view_num != 7) {
    if (!setting_debugout_runquiet) printf("DRMVSNET wrong number of views (%d) at id = %d\n", view_num, id);
    return;
  }
  if (view_num < dr_mvsnet_view_num) {
    if (!setting_debugout_runquiet) printf("DRMVSNET not enough views (%d < %d) at id = %d\n", view_num, dr_mvsnet_view_num, id);
    return;
  }

  if (!setting_debugout_runquiet) {
    printf("DRMVSNET new KF at id = %d\n", id);
    printf("DRMVSNET Frame shell id = %d\n", fh->shell->id);
    printf("DRMVSNET Frame Hessian frameID = %d, idx = %d\n", fh->frameID, fh->idx);
  }

  if (!tandem_backend->Ready()) {
    if (linearizeOperation) {
      tandem_backend->Wait();
    } else {
      if (!setting_debugout_runquiet) printf("TANDEM backend is not ready at id = %d. Will just return!\n", id);
      return;
    }
  }
  ...
  std::vector<cv::Mat> cam_to_worlds_in;
  {
    ...
    for (unsigned int i = 0; i < frameHessians.size(); i++) {
      bgrs_in.emplace_back(height, width, CV_8U, frameHessians[i]->image_bgr);
      cam_to_worlds_in.emplace_back(4, 4, CV_32F, 0.0f);
      auto const &cam_to_world = frameHessians[i]->shell->camToWorld.matrix();
      for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
          cam_to_worlds_in.back().at<float>(r, c) = (float) cam_to_world(r, c);
        }
      }
    }
  }

  // Quantile of depth values
  float depth_quantile;
  int key_quantile;
  {
    boost::unique_lock<boost::mutex> ct_lock(coarseTrackerSwapMutex);
    depth_quantile = get_idepth_quantile(coarseTracker->pc_n_nodia, coarseTracker->pc_idepth_nodia, 0.2f);
  }
  if (!setting_debugout_runquiet) printf("Checking idepth nodia for quantile: n=%6d, depth=%10.5f\n", coarseTracker->pc_n_nodia, depth_quantile);


  if (dr_timing) id_time = dr_timer.start_timing("tandem_backend->CallAsync");
  tandem_backend->CallAsync(
      view_num,
      index_offset,
      corrected_ref_index,
      bgrs_in,
      ...
      dr_async_depth_min,
      3.0f * depth_quantile,
      cam_to_worlds_in[corrected_ref_index]
  );
  if (dr_timing) dr_timer.end_timing("tandem_backend->CallAsync", id_time, !setting_debugout_runquiet);
...
void FullSystem::makeNonKeyFrame(FrameHessian *fh) {
  ...
  traceNewCoarse(fh);
  if (debugSaveDepthImages) coarseTracker->saveNKFDepthMap();
  delete fh;
}

void FullSystem::makeKeyFrame(FrameHessian *fh, float *rgbd_depth) {
  ...
  // Refine the search segment of the immature points
  traceNewCoarse(fh);
  ...
  // Note the idx here is the keyframe id
  ...
  {
    boost::unique_lock<boost::mutex> lock_coarse_tracker(tandem_backend->GetTrackingDepthMapMutex());
    boost::unique_lock<boost::mutex> crlock(coarseTrackerSwapMutex);
    TandemCoarseTrackingDepthMap const *ptr = tandem_backend->GetTrackingDepthMap();

    coarseTracker_forNewKF->makeK(&Hcalib);
    coarseTracker_forNewKF->setCoarseTrackingRef(frameHessians, frameHessians.back(), false, ptr, &dr_timer);


    coarseTracker_forNewKF->debugPlotIDepthMap(&minIdJetVisTracker, &maxIdJetVisTracker, outputWrapper);
    coarseTracker_forNewKF->debugPlotIDepthMapFloat(outputWrapper);
    if (debugSaveDepthImages) coarseTracker_forNewKF->saveKFDepthMap();
  }
  ...
  // =========================== Save optimization window =========================
  optimization_windows.emplace_back(std::vector<int>(frameHessians.size()));
  for (unsigned int i = 0; i < frameHessians.size(); i++) {
    optimization_windows.back()[i] = frameHessians[i]->shell->id;
  }
  ...
  // =========================== Marginalize Frames =========================
    ...
    for (unsigned int i = 0; i < frameHessians.size(); i++)
        ...
        {
            // free(frameHessians[i]->image_bgr);
...
void FullSystem::initializeFromRGBDInitializer(FrameHessian *newFrame, RGBDepth *rgbd_detph) {
  boost::unique_lock<boost::mutex> lock(mapMutex);

  // add firstframe.
  FrameHessian *firstFrame = coarseRgbdInitializer->firstFrame;
  assert(firstFrame->shell->id == 0);
  firstFrame->idx = frameHessians.size();
  frameHessians.push_back(firstFrame);
  ...
  std::vector<PointHessian *> selectedPoints;
  selectedPoints.reserve(wG[0] * hG[0] * 0.2f);
  ...
  // randomly sub-select the points I need.
  float keepPercentage = setting_desiredPointDensity / firstFrame->pointHessians.size();

  if (!setting_debugout_runquiet)
    printf("Initialization: keep %.1f%% (need %d, have %d)!\n", 100 * keepPercentage,
           (int) (setting_desiredPointDensity), (int) firstFrame->pointHessians.size());

  int leftPoints = std::max(firstFrame->pointHessians.size() - setting_desiredPointDensity, 0.0f);
  float keepPercentageImmature = 0.1 * setting_desiredImmatureDensity / leftPoints;

  if (!setting_debugout_runquiet)
    printf("Left: %d try to get keep %.1f%% as immature points)!\n", leftPoints, 100 * keepPercentageImmature);


  std::vector<ImmaturePoint *> immaturePointsToKeep;
  std::vector<ImmaturePoint *> immaturePointsToDelete;

  for (int i = 0; i < firstFrame->pointHessians.size(); i++) {
    if (rand() / (float) RAND_MAX > keepPercentage) {
      if (rand() / (float) RAND_MAX > keepPercentageImmature) {
        immaturePointsToDelete.push_back(firstFrame->immaturePoints[i]);
        continue;
      }

      immaturePointsToKeep.push_back(firstFrame->immaturePoints[i]);
      firstFrame->immaturePoints[i]->lastTraceStatus = IPS_GOOD;
      continue;
    }

    immaturePointsToDelete.push_back(firstFrame->immaturePoints[i]);

    PointHessian *ph = firstFrame->pointHessians[i];

    PointHessian *phCopy = new PointHessian(*ph);

    selectedPoints.push_back(phCopy);

    ef->insertPoint(phCopy);
  }

  for (int i = 0; i < immaturePointsToDelete.size(); ++i) {
    delete immaturePointsToDelete[i];
  }

  firstFrame->pointHessians = selectedPoints;
  firstFrame->immaturePoints = immaturePointsToKeep;

  SE3 firstToNew = coarseRgbdInitializer->thisToNext;
  ...
  if (!setting_debugout_runquiet) {
    printf("INITIALIZE FROM RGBD INITIALIZER (%d pts)!\n", (int) firstFrame->pointHessians.size());
  }
}
  ...
  for (int y = patternPadding + 1; y < hG[0] - patternPadding - 2; y++)
    for (int x = patternPadding + 1; x < wG[0] - patternPadding - 2; x++) {
      ...
      if (!std::isfinite(impt->energyTH)) delete impt;
        ...
        if (gtDepth != nullptr && gtDepth[i] > 1e-5)
          impt->idepth_max = impt->idepth_min = 1.0f / gtDepth[i];
