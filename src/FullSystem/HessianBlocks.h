...
#define SCALE_IDEPTH_VF 1000000.0f		// scales internal value to idepth.
#define SCALE_XI_ROT_VF 1000000.0f
#define SCALE_XI_TRANS_VF 1000000.0f
#define SCALE_F_VF 1000000.0f
#define SCALE_C_VF 1000000.0f
#define SCALE_W_VF 1000000.0f
#define SCALE_A_VF 1000000.0f
#define SCALE_B_VF 1000000.0f

#define SCALE_IDEPTH_INVERSE_VF (1.0f / SCALE_IDEPTH_VF)
#define SCALE_XI_ROT_INVERSE_VF (1.0f / SCALE_XI_ROT_VF)
#define SCALE_XI_TRANS_INVERSE_VF (1.0f / SCALE_XI_TRANS_VF)
#define SCALE_F_INVERSE_VF (1.0f / SCALE_F_VF)
#define SCALE_C_INVERSE_VF (1.0f / SCALE_C_VF)
#define SCALE_W_INVERSE_VF (1.0f / SCALE_W_VF)
#define SCALE_A_INVERSE_VF (1.0f / SCALE_A_VF)
#define SCALE_B_INVERSE_VF (1.0f / SCALE_B_VF)
...
struct FrameHessian
{
    ...
    std::shared_ptr<dvo::core::RgbdImagePyramid> dvo_img;
    ...
    inline void setVirtualFrameStateScaled(const Vec10 &state_scaled)
    {

        this->state_scaled = state_scaled;
        state.segment<3>(0) = SCALE_XI_TRANS_INVERSE_VF * state_scaled.segment<3>(0);
        state.segment<3>(3) = SCALE_XI_ROT_INVERSE_VF * state_scaled.segment<3>(3);
        state[6] = SCALE_A_INVERSE_VF * state_scaled[6];
        state[7] = SCALE_B_INVERSE_VF * state_scaled[7];
        state[8] = SCALE_A_INVERSE_VF * state_scaled[8];
        state[9] = SCALE_B_INVERSE_VF * state_scaled[9];
    ...
    inline void setVirtualFrameEvalPT_scaled(const SE3 &worldToCam_evalPT, const AffLight &aff_g2l)
    {
        Vec10 initial_state = Vec10::Zero();
        initial_state[6] = aff_g2l.a;
        initial_state[7] = aff_g2l.b;
        this->worldToCam_evalPT = worldToCam_evalPT;
        setVirtualFrameStateScaled(initial_state);
        setStateZero(this->get_state());
    };
    ...
    inline FrameHessian(dvo::core::RgbdCameraPyramid& camera, cv::Mat intensity, cv::Mat depth)
    {
        instanceCounter++;
        flaggedForMarginalization=false;
        frameID = -1;
        ...
        debugImage=0;
        dvo_img.reset(new dvo::core::RgbdImagePyramid(camera, intensity, depth));
    };
