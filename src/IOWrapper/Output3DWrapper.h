...
public:
        ...
        virtual void pushDrKfImage(unsigned char * bgr) {}
        virtual void pushDrKfDepth(float const* image, float depth_min, float depth_max) {}
        virtual void pushDrKfConfidence(float const* image) {}
        virtual void publishDrframes(
                unsigned char* image, float* depth, float* confidence, SE3 camToWorld, CalibHessian* HCalib) {}

        virtual void pushFusionKfImage(unsigned char const* image) {}
        virtual void pushFusionKfDepth(float const* image, float depth_min, float depth_max) {}

        virtual void pushDrMesh(size_t num, float const* vert, float const* cols) {}
        ...
        /* call on save mesh */
        virtual bool should_save_mesh() {return true;}
